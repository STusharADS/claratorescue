...
def get_distance_from_esp32():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            current_distance = data.get("distance", -1.0)
            current_button = data.get("button", "N/A")
...
def describe_with_gemini(frame):
    ...
    try:
        ...
        response = gemini_model.generate_content(["From a first person perspective describe the scene in this image and include any prominent text.limi the entire response to 20 words. ", img])
