...
cap = cv2.VideoCapture("http://10.125.129.53:8080/video")
...
def describe_with_gemini(frame):
    ...
    try:
        ...
        response = gemini_model.generate_content(["describe the scene in this image and read any visible text.Limit the entire response to 40 words and directly start saying do not say I am describing or something like that. ", img])
...
def draw_buttons(frame):
    h, w, _ = frame.shape
    BUTTON_WIDTH = w // 5  # Dynamically use w in case it's not 640
    BUTTON_Y = h - BUTTON_HEIGHT
    # Quit button
    cv2.rectangle(frame, (QUIT_RECT[0], BUTTON_Y), (QUIT_RECT[2], h), (200, 200, 200), -1)
    cv2.putText(frame, "Quit", (QUIT_RECT[0] + 35, BUTTON_Y + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
    
    # Register face button
    cv2.rectangle(frame, (REGISTER_FACE_RECT[0], BUTTON_Y), (REGISTER_FACE_RECT[2], h), (200, 200, 200), -1)
    cv2.putText(frame, "Reg Face", (REGISTER_FACE_RECT[0] + 5, BUTTON_Y + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    
    # Register object button
    cv2.rectangle(frame, (REGISTER_OBJECT_RECT[0], BUTTON_Y), (REGISTER_OBJECT_RECT[2], h), (200, 200, 200), -1)
    cv2.putText(frame, "Reg Obj", (REGISTER_OBJECT_RECT[0] + 5, BUTTON_Y + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    
    # Describe button
    cv2.rectangle(frame, (DESCRIBE_RECT[0], BUTTON_Y), (DESCRIBE_RECT[2], h), (200, 200, 200), -1)
    cv2.putText(frame, "Describe", (DESCRIBE_RECT[0] + 5, BUTTON_Y + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)

    # Voice Input button
    cv2.rectangle(frame, (VOICE_INPUT_RECT[0], BUTTON_Y), (VOICE_INPUT_RECT[2], h), (200, 200, 200), -1)
    cv2.putText(frame, "Voice Input", (VOICE_INPUT_RECT[0] + 5, BUTTON_Y + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
...
while True:
    ...
    h, w, _ = frame.shape  # Get actual h and w here
    BUTTON_WIDTH = w // 5  # Dynamic width in case w != 640
    BUTTON_Y = h - BUTTON_HEIGHT
    QUIT_RECT = (0, BUTTON_Y, BUTTON_WIDTH, h)
    REGISTER_FACE_RECT = (BUTTON_WIDTH, BUTTON_Y, 2 * BUTTON_WIDTH, h)
    REGISTER_OBJECT_RECT = (2 * BUTTON_WIDTH, BUTTON_Y, 3 * BUTTON_WIDTH, h)
    DESCRIBE_RECT = (3 * BUTTON_WIDTH, BUTTON_Y, 4 * BUTTON_WIDTH, h)
    VOICE_INPUT_RECT = (4 * BUTTON_WIDTH, BUTTON_Y, 5 * BUTTON_WIDTH, h)
    ...
    if register_object_flag:
        ...
        while capture_active:
            ...
            if input_key == 27:  # Esc
                ...
                try:
                    response = gemini_model.generate_content(["Provide a concise but detailed description of the main object in this image for use in object detection. Try to keep description in less than 30 words", img])
    ...
    if voice_input_flag:
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                rgb_frame = cv2.cvtColor(snapshot, cv2.COLOR_BGR2RGB)
                img = Image.fromarray(rgb_frame)
                response = gemini_model.generate_content(["Answer the given query in 200 words or less:", prompt, img])
                description = response.text
